...
# ─────────────────────────────────────────
# Минимальные цены — отсекаем аксессуары
# кабели и крепления стоят дёшево, реальное
# железо дороже этих порогов
# ─────────────────────────────────────────
MIN_PRICES = {
    "GPU": 300,
    "CPU": 100,
...
class EbayClient:
    ...
    def search(
        ...
        # Фильтр: только новые товары + минимальная цена (отсекаем аксессуары)
        min_price = MIN_PRICES.get(category, 50)
        if only_new:
            params["filter"] = f"conditions:{{NEW}},price:[{min_price}..],priceCurrency:USD"
        else:
        ...
        log.info(f"Поиск на eBay {region}: '{query}' (категория {category}, мин. цена ${min_price})")
...
# ─────────────────────────────────────────
# Быстрый тест:
# PYTHONPATH=etl/extractors python etl/extractors/ebay.py
# ─────────────────────────────────────────
